...
select iif(stockQuantity<30,'kam', 'yetarli') as zaxira from Products

-- 8. Using Customers table, find the total number of customers in each country.

select city, count(*) as mijozlar from customers group by city
...
select year(OrderDate)as year , avg(TotalAmount) as OrderCount from Orders group by OrderDate
...
select top 3 CustomerID,sum(TotalAmount) as totalspent FROM Invoices group by CustomerID Order by totalspent
