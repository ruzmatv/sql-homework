...
select Products.ProductName as nomi,
...
select departments.departmentName as bolimlar,
Employees.name as xodimlar
from departments
join
...
select Products.ProductName as mahsulot_nomi,
suppliers.supplierName as yetkazib_beruvchilar
from Products
join 
...
select Orders.customerId as zakazlar,
customers.firstname as nomi
from Orders
join
...
select students.name as studentlar,
      courses.courseName as kurslar
	  from students
	  cross join
	  courses 
...
SELECT 
    Employees.name AS xodim_ismi,
    Departments.departmentName AS bolim_nomi
FROM 
    Employees
JOIN 
...
SELECT 
    Students.name AS student_nomi,
    Enrollments.courseId AS kurs_id
FROM 
    Students
JOIN 
...
SELECT 
    Orders.orderId AS buyurtma_id,
    Payments.paymentId AS tolov_id,
    Payments.amount AS summa
FROM 
    Orders
JOIN 
...
SELECT 
    Employees.name AS xodim_ismi,
    Departments.departmentName AS bolim_nomi,
    Employees.DepartmentID AS xodim_bolimID,
    Departments.DepartmentID AS bolim_ID
FROM 
    Employees
CROSS JOIN 
    Departments
...
SELECT 
    Orders.orderId,
    Orders.productId,
    Orders.quantity AS buyurtma_miqdori,
    Products.stockQuantity AS ombor_miqdori,
    Products.productName
FROM 
    Orders
JOIN 
...
SELECT 
    Customers.FirstName AS mijoz_nomi,
    Sales.productId AS mahsulot_id,
    Sales.saleAmount AS sotuv_summasi
FROM 
    Customers
JOIN 
...
SELECT 
    Students.name AS student_nomi,
    Courses.courseName AS kurs_nomi
FROM 
    Students
JOIN 
    Enrollments ON Students.studentId = Enrollments.studentId
...
SELECT 
    Products.ProductName AS mahsulot_nomi,
    Suppliers.SupplierName AS yetkazib_beruvchi_nomi
FROM 
    Products
JOIN 
    Suppliers ON Products.SupplierID = Suppliers.SupplierID
...
SELECT 
    e1.name AS xodim1,
    e2.name AS xodim2,
    e1.DepartmentID AS bolim1,
    e2.DepartmentID AS bolim2
FROM 
    Employees e1
JOIN 
...
SELECT 
    manager.employeeId AS manager_id,
    manager.name AS manager_ismi,
    manager.salary AS manager_maoshi,
    emp.employeeId AS xodim_id,
    emp.name AS xodim_ismi,
    emp.salary AS xodim_maoshi
FROM 
    Employees emp
JOIN 
    Employees manager ON emp.managerId = manager.employeeId
WHERE 
