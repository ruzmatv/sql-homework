...
SELECT 
    year1,
    CASE 
        WHEN LEN(REPLACE(CAST(Max1 AS VARCHAR), '0', '')) >= LEN(REPLACE(CAST(Max2 AS VARCHAR), '0', '')) 
             AND LEN(REPLACE(CAST(Max1 AS VARCHAR), '0', '')) >= LEN(REPLACE(CAST(Max3 AS VARCHAR), '0', '')) 
            THEN Max1
        WHEN LEN(REPLACE(CAST(Max2 AS VARCHAR), '0', '')) >= LEN(REPLACE(CAST(Max1 AS VARCHAR), '0', '')) 
             AND LEN(REPLACE(CAST(Max2 AS VARCHAR), '0', '')) >= LEN(REPLACE(CAST(Max3 AS VARCHAR), '0', '')) 
            THEN Max2
        ELSE Max3
    END AS max_without_zeros
FROM 
    TestMax;
...
SELECT 
    ...
    SUBSTRING(
        FullName,
        LEN(FullName) - CHARINDEX(' ', REVERSE(FullName)) + 2,
...
SELECT string, STRING_AGG(sequenceNumber, ', ') AS CombinedValues
FROM DMLTable
GROUP BY string;
...
SELECT
    ...
    CAST(100.0 * SUM(CASE WHEN DATEDIFF(year, hire_date, GETDATE()) > 3 THEN 1 ELSE 0 END) / COUNT(*) AS DECIMAL(5,2)) AS PercentageMoreThan3Years
FROM Employees
...
WITH ExperienceStats AS (
    SELECT
        job_description,
        MAX(experience_years) AS MaxExp,
        MIN(experience_years) AS MinExp
    FROM Personal
...
SELECT
    ...
    p.experience_years,
    CASE
        WHEN p.experience_years = es.MaxExp THEN 'Most Experienced'
        WHEN p.experience_years = es.MinExp THEN 'Least Experienced'
    END AS ExperienceType
FROM Personal p
JOIN ExperienceStats es
    ON p.job_description = es.job_description
WHERE p.experience_years = es.MaxExp
   OR p.experience_years = es.MinExp
ORDER BY p.job_description, ExperienceType DESC;
...
SELECT
    -- Bosh harflar (Uppercase)
    (SELECT STRING_AGG(SUBSTRING(@str, number, 1), '') WITHIN GROUP (ORDER BY number)
     FROM master..spt_values
     WHERE type = 'P' AND number BETWEEN 1 AND LEN(@str)
       AND SUBSTRING(@str, number, 1) LIKE '[A-Z]') AS UppercaseLetters,

    -- Kichik harflar (Lowercase)
    (SELECT STRING_AGG(SUBSTRING(@str, number, 1), '') WITHIN GROUP (ORDER BY number)
     FROM master..spt_values
     WHERE type = 'P' AND number BETWEEN 1 AND LEN(@str)
       AND SUBSTRING(@str, number, 1) LIKE '[a-z]') AS LowercaseLetters,

    -- Raqamlar (Numbers)
    (SELECT STRING_AGG(SUBSTRING(@str, number, 1), '') WITHIN GROUP (ORDER BY number)
     FROM master..spt_values
     WHERE type = 'P' AND number BETWEEN 1 AND LEN(@str)
       AND SUBSTRING(@str, number, 1) LIKE '[0-9]') AS Numbers,

    -- Boshqa belgilar (Others)
    (SELECT STRING_AGG(SUBSTRING(@str, number, 1), '') WITHIN GROUP (ORDER BY number)
     FROM master..spt_values
     WHERE type = 'P' AND number BETWEEN 1 AND LEN(@str)
       AND SUBSTRING(@str, number, 1) NOT LIKE '[A-Za-z0-9]') AS OtherCharacters;
    ...
    studentID,
    grade,
    SUM(grade) OVER (ORDER BY id ROWS UNBOUNDED PRECEDING) AS RunningTotal
FROM Students
ORDER BY StudentID;
...
SELECT SUM(
    CASE
        WHEN equation LIKE '%+%' THEN
            ...
            CAST(SUBSTRING(equation, CHARINDEX('+', equation) + 1, LEN(equation)) AS FLOAT)
