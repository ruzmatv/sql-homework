...
select max(Salary)as Sales_ from Employees
...
select * from Employees
select  departmentName,count(*) as number_of_People from Employees group by departmentName
...
select * from Sales
select customerID,sum(SaleAmount)as mijozlar from sales group by customerID
...
select DepartmentName,avg(Salary) as salary_price from Employees group by departmentName
...
select category, avg(Price) as narx from Products group by category
...
SELECT
    ...
    MIN(TotalAmount) AS LeastPurchaseAmount
FROM Orders
GROUP BY CustomerID;
